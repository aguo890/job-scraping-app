...
def test_samples():
    try:
        ...
        # Select the specific companies mentioned in the verification report
        samples = [
            'Opendoor',    # Moved to Lever
            'Plaid',       # Moved to Greenhouse
            'Deliveroo',   # Moved to Ashby
            'Hex',         # Greenhouse slug 'hextechnologies'
            'Snowflake',   # Ashby check
            'Retool',      # Greenhouse check
            'Shield AI'    # Lever check
        ]
        ...
        for company in companies_to_test:
            logger.info(f"Testing fetch for {company['name']} ({company['ats']})...")
            try:
